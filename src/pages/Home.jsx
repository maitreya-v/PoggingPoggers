      ...
      async function callAPI() {
        try {
          const response = await fetch(
            "https://50ab-14-139-125-227.ngrok-free.app/message",
            {
              method: "POST",
                  ...
                  <div className="botMessageMainContainer">
                    <div className="botMessageWrapper">
                      ...
                      {chat.botMessage ? (
                        <div id="botMessage">
                          {/* <BotResponse
                            response={chat.botMessage.message}
                            // items={chat.botMessage.items}
                            chatLogRef={chatLogRef}
                          /> */}
                          <Box sx={{ width: "100%" }}>
                            <Typography>{chat.botMessage.message}</Typography>
                          </Box>
                          <Box>
                            {chat.botMessage.items ? (
                              <Grid container spacing={2}>
                                {chat.botMessage.items.map((card, index) => (
                                  <Grid
                                    key={index}
                                    item
                                    xs={12}
